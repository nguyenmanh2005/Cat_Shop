  ...
  useEffect(() => {
    ...
    const fetchFavorites = async () => {
      ...
      try {
        ...
        if (ignore) return;
        setError(err?.message || "Unable to load favorites list.");
      } finally {
        if (!ignore) {
          setLoading(false);
        }
      ...
      <main className="flex-1 p-6 animate-fade-in">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-8 animate-fade-in-down">
            <div>
              <h1 className="text-3xl font-bold text-foreground">Favorite Pets</h1>
              <p className="text-muted-foreground mt-2">
                Track products you've marked with a heart for easy access.
              </p>
            </div>
          </div>
          ...
          {loading ? (
            <div className="flex justify-center items-center h-64">
              <div className="flex flex-col items-center gap-4">
                ...
                <div className="text-lg text-muted-foreground animate-pulse">Loading favorites list...</div>
              </div>
            </div>
          ) : error ? (
