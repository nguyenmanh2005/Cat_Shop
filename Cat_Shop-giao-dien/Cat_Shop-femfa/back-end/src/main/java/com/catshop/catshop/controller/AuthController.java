...
public class AuthController {
    ...
    public ResponseEntity<ApiResponse<TokenResponse>> verifyMfa(
        ...
        if (code.matches("^[A-Z0-9]{4}-[A-Z0-9]{4}$")) {
            ...
            // Parse thành int (leading zero sẽ bị mất, nhưng TOTP code thực tế là số nguyên)
            try {
                ...
                log.warn("⚠️ Invalid MFA code format: {}", code);
                ok = false;
            } catch (Exception e) {
                log.warn("⚠️ Error verifying MFA code: {}", e.getMessage());
                ok = false;
            }
        } else {
            throw new BadRequestException("Mã xác thực không đúng định dạng. Vui lòng nhập mã 6 số từ Google Authenticator hoặc backup code (XXXX-XXXX).");
        }
