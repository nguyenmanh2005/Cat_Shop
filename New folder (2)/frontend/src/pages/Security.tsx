...
const Security = () => {
  ...
  // Quản lý thiết bị & cảnh báo bảo mật
  ...
  useEffect(() => {
    ...
    if (user?.email) {
      ...
      // Lấy deviceId hiện tại trên trình duyệt (được authService sử dụng khi đăng nhập)
      try {
        const storedDeviceId = localStorage.getItem("cat_shop_device_id");
        if (storedDeviceId) {
          setCurrentDeviceId(storedDeviceId);
        }
      } catch (err) {
        console.error("Không thể đọc deviceId từ localStorage:", err);
      }
  ...
  const handleRemoveDevice = async (deviceId: number) => {
    ...
    if (!window.confirm("Bạn có chắc muốn đăng xuất thiết bị này? Bạn sẽ cần đăng nhập lại (và có thể cần OTP) trên thiết bị đó.")) {
      return;
    }
  ...
  const handleRemoveAllDevices = async () => {
    ...
    if (!window.confirm("Bạn có chắc muốn đăng xuất tất cả thiết bị khác? Bạn sẽ cần đăng nhập lại trên các thiết bị đó (có thể cần OTP).")) {
      return;
    }
  ...
  // Cập nhật cảnh báo bảo mật khi danh sách thiết bị hoặc device hiện tại thay đổi
  useEffect(() => {
    ...
    devices.forEach((device: any) => {
      if (device.lastLogin) {
        ...
        // Thiết bị đăng nhập trong 24h qua và KHÔNG phải thiết bị hiện tại → coi là thiết bị khác
        const isOtherDevice =
          !!device.deviceId &&
          !!currentDeviceId &&
          device.deviceId !== currentDeviceId;

        if (hoursAgo < 24 && isOtherDevice) {
          alerts.push({
            type: "new_device",
            message: `Đăng nhập từ thiết bị khác: ${parseUserAgent(device.userAgent || "Thiết bị không xác định")}`,
    ...
    // Nếu vừa phát hiện thiết bị khác đăng nhập, hiển thị toast cảnh báo
    if (alerts.length > 0) {
      toast({
        title: "Cảnh báo bảo mật",
        description: "Phát hiện đăng nhập từ thiết bị khác trong 24 giờ gần đây. Vui lòng kiểm tra lại danh sách thiết bị.",
        variant: "destructive",
      });
    }
  }, [devices, currentDeviceId, toast]);
  ...
  // Format IP + hostname cho dễ đọc
  const formatIpDisplay = (ip?: string | null, hostName?: string | null) => {
    if (!ip) return "";
              ...
              {loadingDevices ? (
                ...
                <div className="space-y-4">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="space-y-1 text-xs text-slate-600">
                              {device.ipAddress && (
                                <div className="flex items-center gap-1">
                                  <Globe className="h-3 w-3" />
                                  <span>{formatIpDisplay(device.ipAddress, device.hostName)}</span>
                                </div>
                              )}
                              {device.lastLogin && (
                                <div className="flex items-center gap-1">
                  ...
                  <p className="text-xs text-muted-foreground">
                    💡 Đăng xuất thiết bị sẽ khiến thiết bị đó phải đăng nhập lại (có thể cần OTP) ở lần truy cập tiếp theo
                  </p>
                </div>
              )}
            </CardContent>
                          ...
                          <div className="flex items-center gap-3 mt-1 text-xs text-slate-500">
                            {device.ipAddress && (
                              <span className="flex items-center gap-1">
                                <Globe className="h-3 w-3" />
                                {formatIpDisplay(device.ipAddress, device.hostName)}
                              </span>
                            )}
                            {device.lastLogin && (
                              <span className="flex items-center gap-1">
