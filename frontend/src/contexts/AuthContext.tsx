...
export const AuthProvider = ({ children }: AuthProviderProps) => {
  ...
  const hydrateUser = async (email: string) => {
    try {
      // Endpoint /users/email/{email} không tồn tại trong backend
      // Tạo user từ token thay vì gọi API
      const tokenInfo = authService.parseAccessToken();
      if (!tokenInfo) {
        console.warn('Cannot parse token, creating user from email only');
        // Tạo user đơn giản từ email
      ...
      const authUser = buildAuthUser(mockProfile, tokenInfo);
      setUser(authUser);
    } catch (error) {
      console.error('Error hydrating user:', error);
      // Nếu có lỗi, vẫn tạo user từ email
      const tokenInfo = authService.parseAccessToken();
      const authUser: AuthUser = {
        email: email,
        fullName: email.split('@')[0],
