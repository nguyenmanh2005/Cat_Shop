...
const ProfilePage = () => {
  ...
  const onVerifyGoogleAuthenticator = async ({ code }: GoogleAuthenticatorFormValues) => {
    ...
    try {
      ...
      // Kiểm tra xem là Google Authenticator (6 số) hay Backup Code
      const isGoogleAuth = /^\d{6}$/.test(cleanCode);
      
      if (!isGoogleAuth) {
        setError("Mã không hợp lệ. Vui lòng nhập 6 số từ Google Authenticator.");
        return;
      }
      
      // Convert sang number cho Google Authenticator
      const codeNumber = parseInt(cleanCode, 10);
      
      await authService.verifyMfa({ email, code: codeNumber });
      reset();
      setError(undefined);
                    ...
                    <div className="space-y-2">
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          alert("Secret key đã được sao chép vào clipboard");
                        }}
                        className="w-full rounded border border-blue-300 bg-blue-100 px-3 py-1 text-xs text-blue-800 hover:bg-blue-200"
                      >
                        Sao chép Secret Key
                      </button>
                    </div>
            ...
            {!showBackupCodeForm && (
              <form className="mt-4 space-y-4" onSubmit={handleSubmit(onVerifyGoogleAuthenticator)}>
                <div>
                  ...
                  <input
                    type="text"
                    className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm text-center tracking-[0.3em] focus:border-blue-500 focus:outline-none"
                    placeholder="123456"
                    maxLength={6}
                    {...register("code", {
                      ...
                      pattern: {
                        value: /^\d{6}$/,
                        message: "Mã phải là 6 số"
                      }
                    })}
                    onChange={(e) => {
                      // Chỉ cho phép nhập số, tối đa 6 số
