...
const createApiInstance = (): AxiosInstance => {
  ...
  instance.interceptors.request.use(
    (config) => {
      ...
      // Thêm X-USER-EMAIL header cho TẤT CẢ request (trừ public endpoints) - backend yêu cầu
      const userEmail = localStorage.getItem('user_email');
      
      // Luôn thêm X-USER-EMAIL nếu có email và không phải public endpoint
      if (userEmail && !isPublicEndpoint) {
        config.headers['X-USER-EMAIL'] = userEmail;
      }
      
      // Thêm API key cho các request đến /api/users (trừ đăng ký và login)
      if (config.url?.includes('/users') && !isPublicEndpoint) {
