...
const LoginForm = ({ onSwitchToRegister, onClose }: LoginFormProps) => {
  ...
  const handleEmailPasswordLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Nếu backend yêu cầu OTP (thiết bị mới) → tự động chuyển sang màn hình nhập OTP
      if (result.requiresOtp) {
        setIsLoading(false); // Tắt loading trước
        setVerificationEmail(email);
        setNeedsVerification(true);
        setVerificationMethod("otp"); // Tự động chuyển sang màn hình OTP
        setOtpSent(true); // Đánh dấu OTP đã được gửi (backend đã gửi rồi)
        setOtpCountdown(120); // Bắt đầu countdown 2 phút
        toast({
          title: "Mã OTP đã được gửi",
          description: result.message || "Vui lòng kiểm tra email để lấy mã OTP. Mã có hiệu lực trong 2 phút.",
        });
        return;
      }

      // Nếu đăng nhập thành công (thiết bị quen, có token) → đăng nhập luôn
      if (result.success) {
        toast({
      ...
      // Trường hợp khác → chuyển sang màn hình chọn phương thức xác minh
      setVerificationEmail(email);
      setNeedsVerification(true);
  ...
  const handleRequestOtp = async () => {
    ...
    // Backend tự động gửi OTP khi login với thiết bị mới
    // Để gửi lại OTP, user cần đăng nhập lại
    toast({
      title: "Gửi lại OTP",
      description: "Vui lòng đăng nhập lại để nhận mã OTP mới. Backend sẽ tự động gửi OTP khi phát hiện thiết bị mới.",
      variant: "default",
    });
  ...
  const handleVerifyGoogleAuthenticator = async () => {
    ...
    try {
      ...
      // Backend yêu cầu code là number
      // googleAuthCode đã được filter chỉ còn số (từ onChange), nên convert sang number
      const codeNumber = parseInt(googleAuthCode, 10);
      
      if (isNaN(codeNumber) || codeNumber.toString().length !== 6) {
        toast({
          title: "Mã không hợp lệ",
          description: "Vui lòng nhập mã Google Authenticator 6 chữ số",
          variant: "destructive",
      ...
      const response = await apiService.post<{
        ...
        code: codeNumber, // Gửi code dạng number
      });
      ...
      const errorMessage = error?.response?.data?.message || error.message || "Đã xảy ra lỗi khi xác thực Google Authenticator";
      toast({
        title: "Lỗi xác thực",
        description: errorMessage,
        variant: "destructive",
          ...
          {!needsVerification ? (
            ...
            <>
              ...
              <div className="text-center space-y-2">
                ...
                <p className="text-sm text-muted-foreground">
                  {!otpSent 
                    ? `Mã OTP sẽ được gửi đến: ${verificationEmail}`
                    : `Mã OTP đã được gửi đến: ${verificationEmail}. Vui lòng nhập mã OTP bên dưới.`}
                </p>
              </div>

              {/* Luôn hiển thị form nhập OTP */}
              <div className="space-y-2">
                ...
                <Input
                  ...
                  maxLength={6}
                  className={`text-center text-2xl tracking-[0.6rem] ${otpCountdown === 0 && otpSent ? 'border-red-300' : ''}`}
                  disabled={isLoading}
                />
                {otpCountdown === 0 && otpSent && (
              ...
              {/* Nếu chưa gửi OTP → hiển thị nút "Gửi mã OTP" */}
              {!otpSent ? (
                <Button 
                  type="button" 
                ...
                /* Nếu đã gửi OTP → hiển thị nút "Xác thực OTP" và "Gửi lại" */
